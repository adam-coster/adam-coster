What's the difference between a hyperlink, a link, a deep link, a <abbr title="Uniform Resource Locator">URL</abbr>, and a <abbr title="Uniform Resource Identifier">URI</abbr>? Collectively, these are all about solving two related problems:

1. Finding a way to unambiguously refer to something; and
2. Providing a way to _get to_ that thing.

## What's a URI?

A <abbr>URI</abbr>, or "Uniform Resource Identifier", is a structured string of text with [well-defined syntax](https://datatracker.ietf.org/doc/html/rfc3986) (making it "uniform") that tries to do as the name describes: provide an "identifier" for a "resource".
...
An "identifier" is some kind of label or name that can be associated with and thus "identify" one resource, specifically, among others.
...
Except for the scheme, all parts of a URI are optional.

The scheme (sometimes called the "protocol") indicates the process by which a reader would associate the URI to its resource. For example, `http` indicates that you'd find the associated resource via a network, using the Hypertext Transfer Protocol. `https` means you'd do the same, but with a security layer. `mailto` indicates that you'd be looking for an email address. `tel` indicates that you're looking for a telephone number. [`geo`](https://www.rfc-editor.org/rfc/rfc5870.html) indicates that you'd be tracking down coordinates on the globe. And on and on.
...
The "authority" part is an identifier for the thing that is responsible for the URIs under its purview (like a website host). Certain kinds of URIs (like `http` URLs) require one, while others (like `mailto`, `tel`, or `geo`) do not. It all depends on how uniqueness is created for a particular scheme and historical happenstance. An email address already has its authority baked in (`mailto:me@example.com` gets the job done, why do something weird like `mailto://example.com/me`?). Telephone numbers use a country code to ensure uniqueness and existed before URIs were invented (so `tel:+12345678999` is more understandable than something like `tel://usa/234/567/8999`). You'll mostly see the authority component for network URLs, where the authority is the web host (like `adamcoster.com` or `127.0.0.1:80`).
...
- `https://adamcoster.com` has the `https` scheme, `adamcoster.com` authority, and no path, query, or fragment
- `https://adamcoster.com:443/blog/links-urls-uris-oh-my?does=nothing#example-paragraph` has the `https` scheme, `adamcoster.com:443` authority, `/blog/links-urls-uris-oh-my` path, `does=nothing#example-paragraph` query, and `#example-paragraph` fragment
- `mailto:me@example.com` has the `mailto` scheme, no authority, and `me@example.com` path
- `geo:13.4125,103.8667` has the `geo` scheme, no authority, and `13.4125,103.8667` path
...
This synonym-ization is probably because so many kinds of software these days will automatically wrap a URL in a clickable link, using the URL as the _text_ of that link (e.g. converting `https://example.com` to `<a href="https://example.com">https://example.com</a>`).
